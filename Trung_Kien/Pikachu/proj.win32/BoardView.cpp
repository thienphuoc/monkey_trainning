...
Sprite* BoardView::addPokemon(int row, int column, int type)
{
	...
	auto pokemon = Sprite::create("pokemons/pokemon1/" + std::to_string(type) + ".jpg");
...
bool BoardView::removePokemon(int row, int column)
{
	...
	for (int i = 0; i < board->getNRows(); i++) {
		for (int j = 0; j < board->getNColumns(); j++) {
			if (board->_pokemons[i][j] != -1) b = true;
		}
	}
	if (!b) Director::getInstance()->replaceScene(HelloWorld::createScene());
	return true;
}
