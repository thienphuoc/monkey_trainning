...
void Pipe::spawnPipe(Scene *scene) {
    ...
    bottomPipe->setPosition(topPipe->getPositionX(), topPipePosition - 70 - topPipe->getContentSize().height);

    auto topPipeAction = MoveTo::create(4 , Point(0, topPipePosition));//speed pipe
    auto bottomPipeAction = MoveTo::create(4 , Point(0, topPipePosition - 70 - topPipe->getContentSize().height));
    ...
    auto pointBody = PhysicsBody::createBox(Size(1, 70));
    ...
    pointNode->setPosition(Point(topPipe->getPositionX(), topPipe->getPositionY() - (topPipe->getContentSize().height / 2) - (70 / 2)));

    scene->addChild(pointNode);

    auto pointNodeAction = MoveTo::create(4, Point(0, topPipe->getPositionY() - (topPipe->getContentSize().height / 2) - (70 / 2)));
